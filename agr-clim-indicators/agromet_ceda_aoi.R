...
# total_period_years <- c(2000,2005)
# base_years <- c(2001,2005)
...
dir_gs <- "gs://cmip6_data/CEDA_CMIP6_Downscaled"

dir_data <- "/mnt/pers_disk/tmp"
dir_res <- "/mnt/pers_disk/agr_clim_ind_ceda"
...
for (vc in seq_along(var_combos)) {
  ...
  # obtain lims of aoi and time periods
  "gsutil cp {ff[[1]][1]} {dir_data}" %>% 
    str_glue() %>% 
    system()
