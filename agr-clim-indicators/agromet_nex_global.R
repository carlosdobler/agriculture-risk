...
# SCRIPT TO CALCULATE INDICATORS GLOBALLY BASED ON NEX DATA
...
tb_vars <- 
  read_csv("/mnt/bucket_mine/misc_data/tb_vars.csv") # NOW IN PROJ FOLDER!
...
for (model in models[5]) {
  ...
  for (vc in seq_along(var_combos[1:2])[2]) {                                     # ******************
    ...
    for (tilerow in 1:nrow(tiles_wland)) {                                      # ******************
